...
namespace PusherDroid
{
    ...
    public class Pusher : EventEmitter, IPusher, ITriggerChannels
    {
        ...
        public static T ParseMessageToObject<T>(string message, T objectLikeThis)
        {
            if (String.IsNullOrWhiteSpace(message))
                throw new ArgumentNullException(nameof(message));

            // param "json" is actually a dynamic so we need to cast it to a string
            return JsonConvert.DeserializeAnonymousType(message, objectLikeThis);
        }
